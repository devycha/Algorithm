...
const fs = require("fs");
/** @type {string[]} */
let [T, ...test] = fs.readFileSync("input.txt").toString().trim().split("\n");
/** @type {number[][]} */
let arr = test.map(a => a.split(" ").map(Number));
...
for (let i = 0; i < arr.length; i += arr[i][0] + 3) {
  ...
  /** @type {number[]} 시작점: i+1 */
  let start = arr[i + 1];
  /** @type {number[]} 종료지점: i + arr[i][0] + 2 */
  let end = arr[i + arr[i][0] + 2];
  /** @type {number[][]} 편의점 위치 배열 */
  let store = arr.slice(i + 2, i + arr[i][0] + 3);
  /**@type {number[]} 편의점을 방문했는지 체크하는 체크리스트 */
  let checkList = new Array(arr[i][0]).fill(0);
